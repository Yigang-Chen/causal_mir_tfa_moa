...
go_emb = None
archs4_emb = None
mean_std_dict_go = None
mean_std_dict_archs4 = None
...
def parse_args():
    ...
    parser.add_argument('--test_mode', action='store_true',
                        help='Run in test mode with limited signatures')
    parser.add_argument('--max_signatures', type=int, default=1000,
                        help='Maximum signatures to process in test mode')
...
def get_vec(X):
    sig2vec = {}
    l1k, S_hit, go_emb_data, archs4_emb_data, mean_std_go_data, mean_std_archs4_data = X
    vec = compute_list_emb_with_data(S_hit, go_emb_data, archs4_emb_data, mean_std_go_data, mean_std_archs4_data)
    sig2vec[l1k] = vec
    return sig2vec

def prepare_data(cpd_train, prt, pert_sig, perttype, cpd2target, sigvec_all):
    positive_l = []
    positive_r = []
    negative_l = []
    negative_r = []
    ...
    print(f"Debug: perttype = {perttype}")
    print(f"Debug: Number of compounds in cpd_train: {len(cpd_train)}")
    print(f"Debug: Number of compounds in cpd2target: {len(cpd2target)}")
    print(f"Debug: Sample cpd_train entries: {cpd_train[:5] if len(cpd_train) > 0 else 'None'}")
    print(f"Debug: Sample cpd2target keys: {list(cpd2target.keys())[:5] if len(cpd2target) > 0 else 'None'}")
    
    # Check perttype signatures
    perttype_sigs = pert_sig[pert_sig['Perturbagen']==perttype]
    print(f"Debug: Number of {perttype} signatures: {len(perttype_sigs)}")
    if len(perttype_sigs) > 0:
        print(f"Debug: Sample {perttype} signature names: {perttype_sigs['Name'].head().tolist()}")
    ...
    # Check overlap between signature compounds and cpd_train
    sig_compounds = set(sig_name_cpd.keys())
    cpd_train_set = set(cpd_train)
    sig_train_overlap = sig_compounds.intersection(cpd_train_set)
    print(f"Debug: Signature compounds: {len(sig_compounds)} unique compounds")
    print(f"Debug: Overlap between signature compounds and cpd_train: {len(sig_train_overlap)} compounds")
    if len(sig_train_overlap) > 0:
        print(f"Debug: Sample overlapping sig-train compounds: {list(sig_train_overlap)[:5]}")
    else:
        print("Debug: NO OVERLAP between signature compounds and cpd_train!")
        print(f"Debug: Signature compounds sample: {list(sig_compounds)[:5]}")
        print(f"Debug: cpd_train sample: {list(cpd_train_set)[:5]}")
    
    # Check for overlap between cpd_train and cpd2target
    cpd2target_set = set(cpd2target.keys())
    overlap = cpd_train_set.intersection(cpd2target_set)
    print(f"Debug: Overlap between cpd_train and cpd2target: {len(overlap)} compounds")
    if len(overlap) > 0:
        print(f"Debug: Sample overlapping compounds: {list(overlap)[:5]}")
    else:
        print("Debug: NO OVERLAP - this is why no training pairs are generated!")
        print(f"Debug: cpd_train sample: {list(cpd_train_set)[:5]}")
        print(f"Debug: cpd2target sample: {list(cpd2target_set)[:5]}")
...
def compute_list_emb_with_data(glist, go_emb, archs4_emb, mean_std_dict_go, mean_std_dict_archs4):
    '''Compute the vector representation (embedding) for an input gene list
       input: list of gene IDs, and required data dictionaries
...
if __name__ == '__main__':
    ...
    for idx in t_sig.index:
        ...
        all_cl.add(cl)
        pert_sig.append([l1k, name, cl, dir, des, ty, l1k])
        tasks.append((l1k, S_hit, go_emb, archs4_emb, mean_std_dict_go, mean_std_dict_archs4))
    
    # Apply test mode if enabled
    if args.test_mode:
        tasks = tasks[:args.max_signatures]
        pert_sig = pert_sig[:args.max_signatures]
        print(f"Test mode: Processing only {len(tasks)} signatures")
    ...
    # Debug: Check what perturbagen types are available
    print("Debug: Available perturbagen types:", pert_sig['Perturbagen'].unique())
    print("Debug: Perturbagen type counts:", pert_sig['Perturbagen'].value_counts().to_dict())
    ...
    for testpar in range(0, 5):
        ...
        input_l, input_r, train_label = prepare_data(cpd_train, testpar, pert_sig, perttype, cpd2target, sigvec_all)
        
        # Check if we have training data
        if len(train_label) == 0:
            print(f"Warning: No training pairs found for fold {testpar}. This may happen in test mode with limited data.")
            print("Skipping this fold. Consider using more signatures with --max_signatures or disable --test_mode for full training.")
            continue
